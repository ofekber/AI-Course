...
class HarryPotterProblem(search.Problem):
            ...
            for i, hpos in enumerate(remaining_horcruxes):
                if hpos == (x, y):
                    if lives > 1 or (x, y) not in death_eater_moves:
                        wizard_actions.append(("destroy", wizard_name, i))
    ...
    def result(self, state, action):
        ...
        for wizard_action in action:
            ...
            if action_type == "destroy":
                horcrux_index = details[0]
                # Collect the index for later removal
                destroy_indices.append(horcrux_index)
        ...
        updated_positions = {}
        for name, current_pos in death_eaters_positions:
            path = self.death_eaters[name]
            idx = path.index(current_pos)
            next_idx = (idx + 1) % (2 * len(path) - 2)
            new_pos = path[next_idx if next_idx < len(path) else 2 * len(path) - next_idx - 2]
    ...
    def h(self, node):
        ...
        idle_penalty = sum(1 for wizard in wizards if wizard[1] == "wait") * 10
